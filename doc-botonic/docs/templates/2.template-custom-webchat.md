...
- **enableAttachments**: Authorizes user media attachments or not (`true|false`). It can also be defined in `userInput.attachments.enable`. The `false` value is set by default.
- **enableUserInput**: Enables the user typing zone (`true|false`). It can also be defined in `userInput`. The `true` value is set by default.
- **Listeners**: Listeners are useful to react to some events when using the webchat.

Here is a list of available styles.
...
The `'Cancel'` value is set by default.
...
### Menu and Menu Button Customization

_Menu button_

You can first customize the persistent menu button with the customMenuButton property.
...
import React from 'react'
import { staticAsset } from '@botonic/react'
import SendButtonIcon from '../assets/plus-large.svg'
...
export const CustomMenuButton = () => (
  <IconContainer>
    <Icon src={staticAsset(SendButtonIcon)} />
  </IconContainer>
)
```
...
To customize the persistent menu, you must use the `CustomPersistentMenu` property.
To do so:

1. Create a `custom-persistentMenu.js` file.
...
To darken the webchat in the background and focus on the persistent menu only, add this example content to the webchat `index.js` file.

```javascript
darkBackgroundMenu: true,
    userInput: {
      persistentMenu: [
        { label: 'Help', payload: 'handoff3' },
        { label: 'See docs', url: 'https://docs.botonic.io' },
        { closeLabel: 'Close' },
      ],
    },
