...
For security and privacy reasons, you may have to avoid storing bot data.
To do so, you must use the variables `sessionStorage` and `localStorage`. 
With `localStorage`, the data doesn't expire whereas with `sessionStorage`, everything is cleared once the webchat window is closed. 

By default the `botonicState` is stored in `localStorage`.

To modify it according to your needs, add:
...
1. Create a `coverComponent.js` in `src/webchat`.
...
```javascript webchat = {
import CustomCover from './coverComponent'
export const webchat = {
