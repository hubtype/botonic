...
| Option             | Description                                                                                                                                          | Parameter     | Type    | Req. |
|--------------------|------------------------------------------------------------------------------------------------------------------------------------------------------|---------------|---------|------|
| `withQueue`        | Perform a handoff along with the ID or name of the queue where the conversation is transferred.                                                      | queueNameOrId | string  | Yes  |
| `withOnFinishPath` | Perform a handoff and specify a payload or a path to trigger the action that the bot will follow after the conversation with the agent has finished. | path          | string  | Yes  |
| `withAgentEmail`   | Perform a handoff along with the agent's email.                                                                                                      | email         | string  | Yes  |
| `withAgentId`      | Perform a handoff along with the agent's ID.                                                                                                         | agentId       | string  | Yes  |
| `withNote`         | Perform a handoff and leave a message for other agents to know more about the case.                                                                  | note          | string  | Yes  |
| `withCaseInfo`     | Perform a handoff and give details about the case.                                                                                                   | caseInfo      | string  | Yes  |
| `withShadowing`    | Perform a handoff while hiding agent's ID and name.                                                                                                  | shadowing     | boolean | No   |
...
|----------|--------|-------------------|
| queues   | String | Name of the queue |
...
| Property | Type   | Description       |
|----------|--------|-------------------|
| agents   | String | Name of the agent |
...
| Property | Type              | Description                                                                                                          |
|----------|-------------------|----------------------------------------------------------------------------------------------------------------------|
| agents   | HubtypeAgentsInfo | Agent's information such as email address, number of cases attending, status (available or not) and last message sent |
...
| vacation_ranges | VacationRange | Agent's vacation start and end dates (timestamp) |
