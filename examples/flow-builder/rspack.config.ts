...
const RSPACK_ENTRIES = {
  ...
  WEBCHAT_BUTTON: 'webchat-button-entry.tsx',
...
const LIBRARY_NAME = {
  ...
  BOT: 'bot',
  WEBCHAT_BUTTON: 'BotonicTriggerButton',
}

const FILENAME = {
  WEBCHAT: 'webchat.botonic.js',
  WEBCHAT_BUTTON: 'webchat-button.botonic.js',
...
function botonicWebchatButtonConfig(mode: Mode): Configuration {
  return {
    optimization: {
      minimize: true,
      minimizer: [minimizerPlugin],
    ...
    devtool: sourceMap(mode),
    target: 'web',
    entry: path.resolve(RSPACK_ENTRIES_DIRNAME, RSPACK_ENTRIES.WEBCHAT_BUTTON),
    output: {
      path: OUTPUT_PATH,
      filename: FILENAME.WEBCHAT_BUTTON,
      library: LIBRARY_NAME.WEBCHAT_BUTTON,
      libraryTarget: UMD_LIBRARY_TARGET,
      libraryExport: 'TriggerButton',
      assetModuleFilename: 'assets/[hash][ext][query]',
    },
    module: {
      rules: [
...
export default function (
  ...
  if (env.target === BotonicTarget.ALL) {
    return [
      ...
      botonicWebchatButtonConfig(argv.mode),
    ]
