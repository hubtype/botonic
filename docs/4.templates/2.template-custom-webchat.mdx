---
name: Custom Webchat
route: /4.templates/2.template-custom-webchat
menu: Templates
---
...
This template shows how to create custom elements for webchat. All the custom components are stored in `src/webchat`.
...
- **persistentMenu**: Optionally, a persistent menu can be accessed from a small button at the input text box. It allows the user to access popular functionalities at any point in the conversation.
- **blockInputs**: Enables you to define which user inputs are not allowed. It may be useful for preventing the user to enter a credit card number.
- **emojiPicker**: Enables the emoji picker. Can be defined also in `userInput.emojiPicker`. The `false` value is set by default.
- **enableAttachments**: Authorizes user media attachments or not (`true|false`). Can be defined also in `userInput.attachments.enable`. The`false` is set by default.
