...
route: /2.getting-started/2.getting-started
---
...
1. Close any terminals/command prompts you may have open.
2. Install the latest  LTS version of [NodeJS installer](https://nodejs.org/). You can also install Yarn.
3. Verify the installation version by running `npm --version` and `node --version`
4. Perform a global installation by running  `sudo npm install -g @botonic/cli` (Mac/Linux) or `npm install -g @botonic/cli` (as admin on Windows).
5. Wait until the CLI is installed.
## Step 2 – Create a Bot
1. Run `botonic new <bot_name>` 
2. Select one of the [templates](/4.templates/templates) available. 
...
2. Enter your credentials (email/password) if you already signed up at hubtype.com or create a new account as recommended. 
3. Give a name to your bot and wait for your project to build.

## Step 5 – Publish Your Bot

1. Once the build is finished, go to the Hubtype dashboard: https://app.hubtype.com/
...
2. Select your bot.
3. Click on the "Settings" tab and then on "Delete this bot".