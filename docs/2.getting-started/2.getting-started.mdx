...
1. Install the latest  LTS version of [NodeJS installer](https://nodejs.org/). You can also install Yarn.
2. Verify the installation version by running `npm --version` and `node --version`. **Note:** You must have at least `node >= 0.10.0` version installed.
