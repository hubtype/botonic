...
- `integrations.js`: Define your integration keys with 3rd party services, like NLP platforms.
...
Let's start by creating an "echo" bot that just repeats what the user says. So if the user says "hello world" the bot will respond with "hello world".
...
Now execute `botonic serve` again and try typing "hello world" 😊
...
Botonic has a NLU module under development, but in the meantime you can use our Dialogflow integration. [Dialogflow](https://dialogflow.com/) is Google's NLU as a service.
...
Once this configuration is set, Botonic will automatically send any user input to Dialogflow and get the intent, which you can then match against your routes.
...
Important Note: Routes are checked in order. To function correctly, you'll want to put the more specific ones first and the more generic at the end.
