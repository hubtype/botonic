...
The `botonicInit` function is always called before the `render()`. With this feature, we can do all the external api calls and use the responses in the `render()` method.

The data returned in this function is available in `this.props`.
...
- **params**: when a [Button](/main-concepts/rich-conversational-markup-language) redirects you to an action, you can pass attributes (parameters) to that action. The params store this information.
- **lastRoutePath**: the last path that the user has gone through.

The `render()` function describes what will be returned as an output of the bot using [RCML](/api/rich-conversational-markup-language). All the variables returned in `botonicInit` are stored in `{this.props}`. In this example, you can see two ways of retrieving these variables, in order of appearance:
...
The essential purpose of Botonic is having full control over the bot. You can enable [ExternalCalls](#ExternalCalls) with the `botonicInit({ input, session, params, lastRoutePath })` method and allowing the [HumanHandoff](#humanhandoff) or [Internationalization](#internationalization) in an easy way.
...
<NextButton title="RCML" href="/main-concepts/rich-conversational-markup-language" />