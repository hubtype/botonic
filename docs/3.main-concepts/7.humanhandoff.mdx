...
> #### NOTE:
>
> The conversation is transferred to Hubtype Desk, so the `queueId` must be the id of a queue inside Hubtype Desk. [Contact us](https://botonic.slack.com/) if you need to integrate with another CRM.
...
When creating a new case you can autoassign this case to a specific agent using `getAvailableAgentsByQueue` as shown below.
...
> #### NOTE:
>
> For getting the agents ordered by the number of cases that are attending in Hubtype, you can use `getAvailableAgents`.
