...
Here’s a simple example of how to transfer the conversation into the queue `Support_Queue`. The next step of the bot will be the action called `SayGoodbye` which will be triggered after receiving the payload `sayGoodbye` (which has to be defined in your `routes.js` file).
...
> #### NOTE:
> Right now the conversation is going to be transferred into Hubtype Desk, so the `queueName` must be the name of a queue inside Hubtype Desk. In the future it will be posible to transfer the chat into Zendesk, Front or other ticketing tools.
