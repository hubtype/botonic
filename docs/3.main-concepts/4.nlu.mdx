...
### **1. Installing the plugin for Botonic NLU**
...
You can now run `botonic serve` to see how the bot runs in the development environment. You can also open the `Botonic Dev Console` by clicking the tab you see in the top-left corner for futher details of the variables stored during the execution.
...
<div style={{display:"flex", justifyContent:"space-between"}}>
  <PrevButton title="Components" href="/main-concepts/components" />
  <NextButton title="Webviews" href="/main-concepts/webviews" />
</div>