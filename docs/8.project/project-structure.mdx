...
route: /8.project/project-structure
---
...
To start a new bot, you must run:
...
You must select a template to create a Botonic bot: Either blank or with components. See the [Templates](4.templates/1.templates) section.
...
Routes are how you turn user inputs into Actions. Edit your `src/routes.js` file to add or remove routes. View [Routes](/5.concepts/17.routes) for more details.

Actions are where you define the behavior of your bot. You can add an Action by creating a new .js file inside `src/actions`. View [Actions](/5.concepts/10.actions) and [Components](/5.concepts/12.components) for more details.
...
The you just have to create a couple of actions that respond to these intents in `src/actions`.
