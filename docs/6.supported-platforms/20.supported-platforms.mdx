---
name: Supported Platforms
route: /6.supported-platforms/20.supported-platforms
---

import { NextButton } from '../components/nextButton.tsx'

import { PrevButton } from '../components/prevButton.tsx'



#  Supported Platforms

Botonic supports several messaging platforms, such as:

[Webchat](#webchat) | [Whatsapp](#whatsapp)  | [Facebook Messenger](#facebook-messenger) | [Telegram](#telegram) | [Twitter](#twitter) | [Imbee](#imbee)

The first steps to embed a bot are common to all platforms:

1. Once your bot is deployed, log in to [app.hubtype.com](https://app.hubtype.com/) and click on the Bot icon.
2. Select your bot and click on Integrate a Messenger.
3. Select the relevant messaging app as described below.

## Webchat

You can integrate your chatbot into your personal webpage. This integration is 100% editable, i.e. you can display all messages with your own style.

1. Give a name to this integration (as you can have multiple webchat integrations with the same bot) and click `Enable Webchat`.
2. Some HTML code will be displayed. Inject this code in your webpage. Note that the script has to be injected in the `header` tag, and the bot initialization in the `body` tag.

For more information, refer to the [Custom Webchat](/4.templates/2.template-custom-webchat) template description.

<details>
<summary>Example</summary>


<img
	src="https://botonic-doc-static.netlify.com/images/webchat_channel.png"
	style={{ display: 'block', margin: 'auto', maxWidth: '100%' }}
/>  

</details>

## Whatsapp

Coming soon.

## Facebook Messenger

Facebook Messenger is one of the best messenger platforms for deploying chatbots with its variety of [components](/5.concepts/12.components) outputs.

3. Connect your Facebook account to Hubtype and accept the permissions.
5. Select which of your Facebook Pages you want to connect. If you don't have one, create one [here](https://www.facebook.com/pages/create/) and click on "refresh".
6. Click on "Connect Page".

A new integration appears on the left side. Click on the Facebook Messenger link and start talking to your bot!

<details>
<summary>Example</summary>
<img
	src="https://botonic-doc-static.netlify.com/images/fb_channel.png"
	style={{ display: 'block', margin: 'auto', maxWidth: '100%' }}
/>

</details>

## Telegram

1. Create a bot from the Telegram app.
2. Add the bot username and token and press the `Connect bot` button.

<details>
<summary>Example</summary>

<img
	src="https://botonic-doc-static.netlify.com/images/tg_channel.png"
	style={{ display: 'block', margin: 'auto', maxWidth: '100%' }}
/>
</details>

## Twitter

- Follow the integration steps as displayed.

Limitation: in your Twitter App, you can only have a unique webhook per environment. So it's important to **check** this option when you want to integrate your bot into your Twitter App.

<details>
<summary>Example</summary>

<img
	src="https://botonic-doc-static.netlify.com/images/twitter_channel.png"
	style={{ display: 'block', margin: 'auto', maxWidth: '100%' }}
/>
</details>

## Imbee

Coming soon.

<div style={{display:"flex", justifyContent:"space-between"}}>
  <PrevButton title="Botonic Settings" href="/main-concepts/botonic-settings" />
  <NextButton title="Tutorials & Examples" href="/tutorials-and-examples/guided-flow" />
</div>
