...
Note that you can specify directly what action has to be triggered when answering a reply with the attribute `path="childStateURL"`. 

After applying this operation to each action file, you will have the ability to obtain the desired flow.

### Working with URL Parameters

Since every action is linked in the same way as URL's, you can also pass additional parameters to them in order to have better control of your responses. So, the following piece of code will fill our params object:
...
<NextButton title="Human Handoff" href="/tutorials-and-examples/human-handoff" />
<PrevButton title="Channels" href="/main-concepts/channels" />
