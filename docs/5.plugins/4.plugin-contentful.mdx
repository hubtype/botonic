---
name: 4. Using Botonic Plugin Contentful
...
* Combinations of the following Botonic UI components: Carousels and Texts. Carousel and Texts buttons can be configured
...
#### References between contents

Your *Text* or *Carousel* buttons may trigger different behaviours depending on the type of its *target* field

* If you assign a Text or Carousel, the button text will be automatically assigned to the *Short Text* field of the target content. When the button is pressed, it will send your bot a payload with value "\<MODEL_TYPE\>$\<ID\>". \<MODEL\_TYPE\> may be *text* or *carousel*, whereas \<ID\> will be the ID of the target.
...
#### Rendering a botonic Text or Carousel

To render a botonic Texts and Carousels with the contents configured at contentful space:
...
3) to render the *Text*s and *Carousel*s assigned to the buttons at the Contentful dashboard,
...
Apart from fetching your contents by ID, you can also search them. From www.contentful.com, you can assign Keywords to your Text, Carousel, URLs and Queue's. Each keyword is a string
...
<div style={{display:"flex", justifyContent:"flex-start"}}>
  <PrevButton title="Botonic Plugin NLU" href="/plugins/plugin-nlu" />
</div>