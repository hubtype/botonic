...
but with little particularities.
...
which contains an scaffold project to start and can be initialized by running:

```bash
botonic new {BOT_NAME} nlu
```

but if you want to take a closer look of how to make it work from scratch, you can follow the next guidelines. We encourage you to go for the two options nevertheless.
...
After having defined this in your project, you just only need to add a bunch of simple text files as `{YOUR_INTENT}.txt` and fill them with the utterances you think that describes the best the intent they represent.
...
So in this example, we have added three different which our bot will know how to understand. This would sound as an obvious statement, 
but the more example utterances you have, the better results you will obtain.

### **Configuring the plugin **
When you feel that your dataset could not be defined better, you can move to the plugin's configuration. It consists of two steps:
...
as well as it permits to configure the parameters that will be run during the training phase.
...
The order of the languages you define in this object, will determine which language have preference when the plugin is not able to correctly identify 
which is the language of a new input.

> #### IMPORTANT NOTE:
> By this moment we only support the following pre-trained word embeddings, so you only be able to train with one of these four, but
> we hope to support as many of them as possible in a near future. For more information, feel free to contact us at [Slack](http://botonic.slack.com).
> * **glove-50d-eng**
> * **glove-100d-eng**
...
### **Training phase **
Now it's time to see how good are the results. It's time to run the training algorithm which will try to do its best in order to generalize on new input phrases.
...
The first time you run this command and if there are no errors during the configuration of the plugin, a download will automatically start in order to have the pre-trained word embeddings available for you.
...
add new samples on the existing files or creating new intents, if you prefer. You can quit the interface by entering `exit()`.
Try to add some new examples and run `npm run train` and `npm run retrain` iteratively and see how your algorithms improves gradually. 
...
can use the intents we previously define to take full advantage when defining new routes.
...
In the previous example, you can see how you can match your inputs by using the intent matcher or if you want a more precise control, 
by using the input matcher. When defining routes, the attributes `intent`, `confidence` and `intents` will be available in runtime in the input object.

## **Going live**

As a second last step, you can now run the command `botonic serve` and see how the bot would be like in the development environment.
...
Finally, deploy your bot with `botonic deploy` and you will have a bot with a personified intent recognition system!  
