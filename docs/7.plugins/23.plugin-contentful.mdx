...
route: /plugins/23.plugin-contentful
menu: Plugins
---
...
5. Create a *Delivery API token* and safely store its value (it grants read access to your account).
    <img
   src="https://botonic-doc-static.netlify.com/images/contentful/delivery-api-key.png"
   style={{ display: 'block', margin: 'auto', maxWidth: '60%' }}
    />

6. Create a *Content Management token* pressing *Generate personal token* and safely store its value (it grants write access to your account).
    <img
