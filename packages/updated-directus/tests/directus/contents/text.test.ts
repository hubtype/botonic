...
const TEXT_WITH_CAROUSEL_FOLLOWUP = '4956d1b7-bfb7-4450-80f9-ede66f8b9d99'
...
test('Test: directus text without buttons with carousel followup', async () => {
  const directus = testDirectus()
  const testText = await directus.text(
    TEXT_WITH_CAROUSEL_FOLLOWUP,
    testContext()
  )
  const followup = testText.common.followUp as Carousel
  expect(followup).toBeInstanceOf(Carousel)
  expect(followup.common.name).toEqual('TEST_CAROUSEL')
  expect(followup.elements).toHaveLength(3)
})
