...
export const Webchat = forwardRef(
  /**
   * @param {WebchatArgs} props
   */
  ...
  const theme = merge(webchatState.theme, props.theme)
    ...
    const host = props.host || document.bodyconst saveWebchatState = webchatState => {
      storage &&
        saveState(
          JSON.parse(
            stringifyWithRegexs({
  ...
  //Load initial state from storage
    useEffect(() => {
      ...
      if (
        shouldKeepSessionOnReload({ initialDevSettings,
        devSettings})
      ) {
        if (messages) {
          messages.forEach(m => {
  ...
  */
    ...
    let mobileStyle = {}
    if (
      isMobile(getThemeProperty(WEBCHAT.CUSTOM_PROPERTIES.mobileBreakpoint))
    ) {
